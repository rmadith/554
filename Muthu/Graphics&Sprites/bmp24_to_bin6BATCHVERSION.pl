...
sub get_filename_without_extension {
    ...
    my $last_slash_index = rindex($filepath, '/');
    my $last_backslash_index = rindex($filepath, '\\');
    my $last_index = ($last_slash_index > $last_backslash_index) ? $last_slash_index : $last_backslash_index;
    my $filename_with_extension = substr($filepath, $last_index + 1);

    # Find the last occurrence of '.' to get the extension
    my $last_dot_index = rindex($filename_with_extension, '.');

    # Extract the filename without extension
    my $filename_without_extension = ($last_dot_index == -1) ? $filename_with_extension : substr($filename_with_extension, 0, $last_dot_index);
...
my $basename = get_filename_without_extension($infile);

$out_folder = $ARGV[1];

$PGMname =  $out_folder . "/" . $basename .".pgm";
$HEXname =  $out_folder . "/" . $basename .".hex";
...
$ROMname = $out_folder . "/" . "BMP_ROM_".$basename.".v";
...
print OUTFILE3 "    \$readmemb(\"$HEXname\",rom);\n\n";
