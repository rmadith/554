...
reset_synch rst(.RST_n(KEY[0]), .clk(clk), .rst_n(inter_rst_n));
reset_synch debug(.RST_n(SW[0]), .clk(clk), .rst_n(debug));
assign rst_n = debug | inter_rst_n;

cpu iCPU(.clk(clk), .rst_n(rst_n), .rdata({{2{1'b0}}, inter_databus}), .wdata(databus), .re(re), .we(we), .addr(addr), .debug(debug));
